...
TEST_CASE("MNISTLoader - Load and Validate Dataset", "[loader]") {
    ...
    SECTION("Validate known training labels") {
        ...
        // The first 10 labels of the MNIST training set are: 5, 0, 4, 1, 9, 2, 1, 3, 1, 4
        ...
        for (size_t i = 0; i < expectedLabels.size(); ++i) {
            ...
            for (int j = 0; j < 10; ++j) {
                if (j == expectedLabel) {
                    REQUIRE(dataset.targets(static_cast<int>(i), j) == Catch::Approx(1.0));
                } else {
                    REQUIRE(dataset.targets(static_cast<int>(i), j) == Catch::Approx(0.0));
                }
            }
        }
    }
    ...
    SECTION("Compare loader contents with raw binary files directly") {
        ...
        std::ifstream imgFile(datasetPath + "train-images.idx3-ubyte", std::ios::binary);
        std::ifstream lblFile(datasetPath + "train-labels.idx1-ubyte", std::ios::binary);
        
        REQUIRE(imgFile.is_open());
        REQUIRE(lblFile.is_open());
        
        // Skip headers
        // Image header: magic (4B), numImages (4B), numRows (4B), numCols (4B) = 16 bytes
        imgFile.seekg(16);
    ...
    SECTION("Error handling for invalid paths") {
        ...
        REQUIRE_THROWS_AS(badLoader.load("non_existent_directory/"), std::runtime_error);
    }
}